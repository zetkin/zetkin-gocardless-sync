    ...
    async init(gc, z, config) {
        ...
        return {
            async syncNumbers() {
                ...
                const toBeNumbered = [];
                people.forEach((p) => {
                    const prefix = p.ext_id.slice(0, 3)
                    const n = parseInt(p.ext_id.slice(3));
                ...
                for (const person of sorted) {
                    person.ext_id = 'UVW' + nextNumber.toString().padStart(6, '0');
                    ...
                    await z.resource('orgs', config.ZETKIN_ORG_ID, 'people', person.id).patch({ ext_id: person.ext_id })
                    nextNumber++;
                }
            ...
            async syncMonth(year, month) {
                ...
                console.log(
                    `   - Loading payments for ${monthLabel}, ${yearLabel}`
                );
